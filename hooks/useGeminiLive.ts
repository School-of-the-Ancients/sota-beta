...
export const useGeminiLive = (
    ...
    const connect = useCallback(async () => {
        ...
        try {
            ...
            if (voiceAccent?.trim()) {
                const accentDirective = `VOICE & DELIVERY GUIDANCE: Maintain a consistent performance using ${voiceAccent}. Do not shift accents, pitch, or presentation away from this specification, and ensure the delivery aligns with the character's identity.`;
                finalSystemInstruction = `${accentDirective}\n\n${finalSystemInstruction}`;
            }
            if (activeQuest) {
