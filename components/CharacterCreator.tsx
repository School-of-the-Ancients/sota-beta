...
If you are not at least 80% confident in their historicity, set verified to false.`;
  ...
  const handleCreate = async () => {
    ...
    try {
      if (!apiKey) {
        throw new Error('Add your API key in Settings to create new ancients.');
      }
      setLoading(true);
  ...
  return (
    <div className="max-w-3xl w-full mx-auto bg-[#202020] p-4 sm:p-6 md:p-8 rounded-2xl shadow-2xl border border-gray-700 relative">
      ...
      <div className={loading ? 'opacity-20 blur-sm' : ''}>
        ...
        {!apiKey && (
          <div className="bg-amber-500/10 border border-amber-400/60 text-amber-100 text-sm px-4 py-3 rounded-lg mb-6">
            Add your Gemini API key in Settings to unlock AI-generated mentors and portraits.
          </div>
        )}
